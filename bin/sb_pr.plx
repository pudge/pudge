...
sub type_start_html {
    ...
    for my $filter (@$filters) {
        my $query_html = encode_entities($filter);
        my $query_url_html = encode_entities(query_url($filter));
        $filter_html .= qq{ <a href="$query_url_html" class="link-gray-dark text-small" title="$query_html">#</a>};
        $c++;
    }
...
sub issue_html {
    ...
    my $reviews = $gh->command(GET => $issue->{pull_request}{url} . '/reviews');
    my $repo = $pr->{base}{repo}{full_name};

    my $assignees = '';
    if (@{$issue->{assignees}}) {
        $assignees = ' &bull; assigned ' . join '', map { user_html($_) } @{$issue->{assignees}};
    }

    my $reviewers = '';
    my %review_states;
    for my $review (@$reviews) {
        push @{$review_states{$review->{state}}}, $review;
    }
    for my $state (sort keys %review_states) {
        my %review_seen;
        $reviewers .= " &bull; \L$state\E "
            . join '',
                map { user_html($_->{user}) }
                grep { !$review_seen{$_->{user}{login}}++ }
                @{$review_states{$state}};
    }
...
<li class="lh-condensed Box-row Box-row--focus-gray navigation-focus">
    ...
    <a href="$pr->{url}" title="API">&bull;</a>
    $labels
    ...
    <div class="mt-1 text-small text-gray">
        created $user$assignees$reviewers
    </div>
</li>
...
sub user_html {
    my($user) = @_;
    return <<EOT;
<a href="$user->{html_url}" class="muted-link"><img src="$user->{avatar_url}" title="\@$user->{login}" alt="\@$user->{login}" class="avatar avatar-small" height="16" width="16"></a>
EOT
}
...
<div style="background-color: #f05624;">
    <a href="https://www.shiftboard.com/"><img width="162" height="36" src="https://www.shiftboard.com/wp-content/uploads/2016/06/Shiftboard_Logo_white.png" alt="Shiftboard" /></a>
    <a href="https://github.com/shiftboard"><svg aria-hidden="true" height="32" version="1.1" viewBox="0 0 16 16" width="32" fill="#fff">
